...
casper.then(function () {   
	...
	this.wait(5000, function() {
		this.click('#domesticDiv .wrap_ts dd.w332 input');
    });
	this.wait(5000, function() {
		this.evaluate(function() {
			$("#domesticDiv .wrap_ts dd.w332 input").val(city);
		}); 
	});
	this.wait(5000, function() {
		$("#domesticDiv dl:first dt").click();
    });
...
casper.then(function () {
	...
	this.wait(4000, function() {// 等待2秒
		...
		fs.write(getNowDate() + "/" + city + "/hotelListTotalPage.json", pages, 'w');
    });
});
...
casper.then(function () {
	...
	casper.repeat(pages,function() {  
		...
		this.wait(waitTime, function () {   
			...
			//写入文件
			fs.write(getNowDate() + "/" + city + "/list/hotelList_"+onPage+".json", val, 'w');
			onPage++;
			this.echo("保存数据成功");
